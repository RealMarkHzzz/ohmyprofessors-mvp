...
import { trackProfessorView } from '@/lib/analytics'
...
export default function ProfessorPage({ params, searchParams }: ProfessorPageProps) {
  const router = useRouter()
  const { slug } = use(params)
  const sp = use(searchParams)
  
  // Deserialize data from search params (passed from server component wrapper)
  const professor: Professor | null = sp.professor ? JSON.parse(decodeURIComponent(sp.professor)) : null
  const initialReviews: Review[] = sp.reviews ? JSON.parse(decodeURIComponent(sp.reviews)) : []
  const ratingDist: Record<number, number> = sp.ratingDist ? JSON.parse(decodeURIComponent(sp.ratingDist)) : {}
  ...
  useEffect(() => {
    if (professor) {
      ...
      trackProfessorView(professor.id, professor.name);
